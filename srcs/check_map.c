...
/*   Updated: 2024/06/19 15:34:33 by claprand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_wall(t_sl *sl)
{
	...
	while (i < sl->width_map)
	{
		if (sl->map[0][i] != '1' || sl->map[sl->nbline - 1][i] != '1')
			return (ft_putstr_fd(ERROR_WALL, 2),
				freetab(sl->map, sl->height_map), 1);
		i++;
	}
	i = 0;
...
{	
	...
	if (sl->nbexit == 0)
	{
		ft_putstr_fd(NO_EXIT, 2);
		return (freetab(sl->map, sl->height_map), 1);
	}
	if (sl->nbitem <= 0)
		return (ft_putstr_fd(ERROR_ITEM, 2),
			freetab(sl->map, sl->height_map), 1);
	return (0);
}
