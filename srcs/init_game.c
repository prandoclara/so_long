...
int	init_mlx(t_sl *sl)
{
	...
	sl->win_ptr = mlx_new_window(sl->mlx_ptr, sl->width_map * IMG_WIDTH,
			sl->height_map * IMG_HEIGHT, "so_long");
	if (!sl->win_ptr)
	{
...
int	load_images(t_sl *sl)
{
	...
	if (!sl->exit)
		return (free_resources(sl), 1);
	sl->item = mlx_xpm_file_to_image(sl->mlx_ptr, ITEM_XPM,
			&sl->width_img, &sl->height_img);
	if (!sl->item)
		return (free_resources(sl), 1);
	sl->player = mlx_xpm_file_to_image(sl->mlx_ptr, PLAYER_FRONT_XPM,
			&sl->width_img, &sl->height_img);
	if (!sl->player)
		return (free_resources(sl), 1);
	return (0);
